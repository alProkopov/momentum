...
async function getWeather() {
  const url = `https://api.openweathermap.org/data/2.5/weather?q=${cityFragment.value}&lang=${hash}&appid=cb776da96964ccf9c3b04f57d3e794af&units=metric`;
  ...
  windFragment.textContent = `${translation.WindSpeed[`${hash}`]}: ${Math.round(data.wind.speed)} ${translation.ms[`${hash}`]}`;
  humidityFragment.textContent = `${translation.Humidity[`${hash}`]}: ${data.main.humidity}%`;
}
